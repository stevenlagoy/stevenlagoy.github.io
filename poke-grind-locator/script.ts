...
function readPokemonJSON() {
    fetch(pokemonJSONPath)
        ...
        .then(data => {
            ...
            (data as PokeJSON[]).filter((poke: PokeJSON) => poke.id && poke.name && poke.type && poke.base_exp && poke.base)
                .forEach((poke: PokeJSON) => {
                    let stats = new Stats(
                        poke.base.HP,
                        poke.base.Attack,
                        poke.base.Defense,
                        poke.base["Sp. Attack"],
                        poke.base["Sp. Defense"],
                        poke.base.Speed
                    )
...
function readMovesJSON() {
    fetch(movesJSONPath)
        ...
        .then(data => {
            ...
            (data as MoveJSON[]).filter((move: MoveJSON) => move.accuracy && move.ename && move.power && move.pp && move.type)
                .forEach((move: MoveJSON) => {
    ...
    fetch(routesJSONPath)
        ...
        .then(data => {
            ...
            interface RouteJSON {
                route_name: string;
                route_number: number;
                available_pokemon: any; // Replace 'any' with a more specific type if available
                [key: string]: any;
            }

            (data as RouteJSON[]).filter((route: RouteJSON) => route.route_name && route.route_number && route.available_pokemon)
                .forEach((route: RouteJSON) => {
...
function addPokemonSelectEntry(pokemon: Pokemon) {
    ...
    else throw new Error("HTML element missing or of improper type.");
}

function addMoveSelectEntry(move: Move) {
    for (let i = 1; i <= 4; i++) {
...
function addPokemonSelectListeners() {
    ...
    const img1          = document.getElementById("pokemon-type-img1") as HTMLImageElement | null;
    const img2          = document.getElementById("pokemon-type-img2") as HTMLImageElement | null;
    const pokeImage     = document.getElementById("pokemon-image") as HTMLImageElement | null;
...
function updateMoveInfo(moveSelectId: string, infoBoxId: string) {
    const select = document.getElementById(moveSelectId) as HTMLSelectElement | null;
...
function getUserPokemon(): Pokemon {
    ...
    return new Pokemon(
        ...
        selectedPokemon.baseExp,
        Number(levelSelect.value),
        selectedPokemon.stats
    );
}

let maxRouteNum: number;
function getMaxRouteNum(): number {
...
let dangerRatio: number;
function getDangerRatio(): number {
...
async function locate() {
    try {
        ...
        let routeWeights: Map<string, number> = new Map<string, number>();
        ...
        if (resultsDiv) {
            ...
            let html = `
                ...
                <p>The name of the Route is listed, followed by the expected experience, then the expected danger on the Route.</p>
                <h3>The best places to grind for levels are:</h3>
                <ol>
                    ${topRoutes.map(({ route, weight, avgDanger }) =>
                        `<li>${route} <span style="color:#888;font-size:0.9em;">(Exp: ${weight.toFixed(2)}, Danger: ${avgDanger.toFixed(2)})</span></li>`
                    ).join('')}
                </ol>
            `;
...
function dangerToMatchup(danger: number): number {
...
function matchupToDanger(matchup: number): number {
