...
import { Encounter, Time } from './Encounter.js';
...
export class Pokemon {
    ...
    level: number;
    stats: Stats;
    constructor(id: number, name: string, type: Type[], baseExp: number, level: number, stats: Stats) {
        this.id = id;
        this.name = name;
        this.type = type;
    ...
    async evaluateRouteMatchup(route: Route, time: Time): Promise<number> {
        return await aggregateMatchupResults(this, route.encounters, time);
    }

    async evaluateRouteExperience(route: Route, time: Time, acceptableMatchup: number): Promise<number> {
...
function compareStats(attackerStats: Stats, defenderStats: Stats): number {
...
async function evaluateMatchup(attacker: Pokemon, defender: Pokemon): Promise<number> {
    const typeMultiplier = await Type.getTypeEffect(attacker.type, defender.type); // e.g. 2, 1, 0.5
    const typeScore = Math.log2(typeMultiplier as number); // 1 for x2, 0 for x1, -1 for x0.5
...
async function calculateExpectedExperience(attacker: Pokemon, defender: Pokemon, defenderLevel: number, acceptableMatchup: number) {
    let matchupScore = await evaluateMatchup(attacker, defender);
    if (matchupScore < acceptableMatchup) return 0;
    let experienceGain = calculateExperienceGain(attacker, defender, defenderLevel);
    return matchupScore * experienceGain;
}

function calculateExperienceGain(winner: Pokemon, loser: Pokemon, loserLevel: number) {
...
async function aggregateExperienceResults(pokemon: Pokemon, encounters: Encounter[], time: Time, acceptableMatchup: number): Promise<number> {
    ...
    const expGains = await Promise.all(
        relevantEncounters.map(async encounter => {
            // Use the encounter's pokemon and level for calculation
            const exp = await calculateExpectedExperience(
                pokemon,
                encounter.pokemon,
                encounter.level ?? encounter.pokemon.level,
                acceptableMatchup
            );
